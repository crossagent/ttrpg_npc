...
### 5. Agent系统 (AgentSystem)
**功能**：管理DM和玩家的AI代理，处理决策生成。
**重要接口**：
- `dm_generate_narrative(game_state: GameState, script: Script) -> str`：DM生成叙述
...
### 2. 回合中DM叙事处理
回合管理器调用Agent系统让DM根据当前游戏状态和剧本生成叙述，推进故事。

```
回合管理器.process_dm_turn()
  |-- Agent系统.dm_generate_narrative(game_state, script) -> 返回DM叙述
...
回合管理器.process_player_turns()
  |-- 循环处理每个玩家:
      ...
      |-- Agent系统.player_decide_action(player_id, context) -> 玩家决策
...
回合管理器.resolve_actions(player_actions)
  |-- 针对每个实质性行动:
      ...
      |-- Agent系统.dm_resolve_action(action, game_state) -> DM解析行动结果
      |-- 如果需要掷骰:
